...
test("owner invites a member who accepts an assigned task and changes its status", async ({ page, request, baseURL }) => {
  ...
  await expect(page.getByText(commentBody)).toBeVisible();
  const evaluationComment = `Livrable excellent ${uniqueId}`;
  await page.getByLabel("Membre évalué").selectOption({ label: "Member E2E" });
  ...
  await page.getByLabel("Mot de passe").fill(password);
  await page.getByRole("button", { name: "Se connecter" }).click();
  await expect(page.getByText(`Nouvelle évaluation sur ${taskName}`)).toBeVisible();
  await page.getByRole("button", { name: "Ouvrir" }).click();
  await expect(page.getByText(evaluationComment)).toBeVisible();
  ...
  await expect(page.getByText("a ajouté une évaluation")).toBeVisible();
});
